...
async function updateMod(config, context, instance, modInfo, fileBuffer) {
    ...
    try {
        ...
        data.append("file", fileBuffer, modInfo.name + "_" + context.nextRelease.version + ".zip");
        const uploadRes = await instance.post(initRes.data.upload_url, data);
...
async function publishMod(config, context, instance, modInfo, fileBuffer) {
    ...
    try {
        ...
        var data = new FormData();
        data.append("file", fileBuffer, modInfo.name + "_" + context.nextRelease.version + ".zip");
        data.append("description", modInfo.description);
        const uploadRes = await instance.post(initRes.data.upload_url, data);
